...
export async function handleDeleteSession(sessionId, req, res) {
    const user = requireAuth(req, res);
    if (!user)
        return;
...
export async function handleListMessages(sessionId, req, res) {
    const user = requireAuth(req, res);
    if (!user)
        return;
    const owned = await requireSessionOwnership(sessionId, user.userId, res);
...
export async function handleCreateMessage(sessionId, req, res) {
    const user = requireAuth(req, res);
    if (!user)
        return;
    const owned = await requireSessionOwnership(sessionId, user.userId, res);
