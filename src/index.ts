 ...
 * @returns 模型返回的完整字符串，失败时返回 undefined
 */
/**
 * 经营事件列表单条数据结构
 */
 ...
 * @returns 结构化事件列表
 */
export async function fetchEventsWithTool(userMsg: string): Promise<EventListPayload> {
  ...
  const tools = [
    {
      ...
      function: {
        ...
        description:
          "当用户查询企业经营事件、签约、收入、回款、验收、预警等数据时，调用此函数返回结构化事件列表",
        parameters: {
          type: "object",
          properties: {
            title: {
    ...
    messages: [
      {
        role: "system",
        content:
          "你是企业经营数据助手。当用户查询经营事件、签约、收入、回款、验收、预警等数据时，必须调用 render_event_list 函数返回结构化数据。如果用户没有提供具体数据，请基于合理假设生成示例数据，确保金额、日期、公司名称看起来真实。",
      },
  ...
  if (toolCall && toolCall.type === "function" && "function" in toolCall) {
    ...
    if (fn.name === "render_event_list") {
      return JSON.parse(fn.arguments) as EventListPayload;
    }
  }
...
// 保留原有非流式调用，向后兼容
export async function zhipuChat(userMsg: string): Promise<string | undefined> {
  try {
    const model = getRequiredEnv("ZHIPU_MODEL");
    ...
    console.error("调用失败：", (err as Error).message);
    return undefined;
  }
}
